...
using Talent.Services.Talent.Domain.Models;
...
namespace Talent.Services.Listing.Controllers
{
    ...
    public class ListingController : Controller
    {
        ...
        // Returns the job record for the specified job...
        ...
        // Copies the current job...
        [HttpPost("copyExistingJob")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "employer, recruiter")]
        public IActionResult CopyExistingJob([FromBody]SingleFieldClass singleFieldClass)
        {
            string id = singleFieldClass.id;
            try
            {
                // Get the current job data...
                var jobData = _jobService.GetJobByIDAsync(id).Result;
                jobData.Id = "";
                // check if it's by same employer...
                if (jobData.EmployerID == _userAppContext.CurrentUserId)
                { 
                    // Create a copy of the retrieved job with the changed below details...
                    jobData.Status = JobStatus.Active;
                    jobData.CreatedOn = DateTime.UtcNow;
                    string newJobID = _jobService.CreateJob(jobData);
                    return Json(new { Success = true, jobData });
        ...
        public async Task<IActionResult> GetEmployerJobs(string employerId=null)
        {
            try
            {
                ...
                var myJobs = (await _jobService.GetEmployerJobsAsync(employerId)).Select(x => new { x.Id, x.Title, x.Summary, x.JobDetails.Location, x.Status, x.ExpiryDate, noOfSuggestions=x.TalentSuggestions!=null && x.TalentSuggestions.Count!=0 ?x.TalentSuggestions.Count:0 });
       ...
       // [HttpGet("getSortedEmployerJobs")]
        [HttpPost("getSortedEmployerJobs")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "employer, recruiter")]
        public async Task<IActionResult> GetSortedEmployerJobs([FromBody] MultipleParametersClass multipleParametersClass)
        {
            int activePage = multipleParametersClass.activePage;
            string sortbyDate = multipleParametersClass.sortbyDate;
            bool showActive = multipleParametersClass.showActive;
            bool showClosed = multipleParametersClass.showClosed;
            bool showDraft = multipleParametersClass.showDraft;
            bool showExpired = multipleParametersClass.showExpired;
            bool showUnexpired = multipleParametersClass.showUnexpired;
        ...
        [HttpPost("closeJob")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "employer, recruiter")]
        public async Task<IActionResult> CloseJob([FromBody]SingleFieldClass singleFieldClass)
        {
            // The argument in this function was "string id" but it has to be a class with
            // the same fields as supplied data in 'ajax' request...
